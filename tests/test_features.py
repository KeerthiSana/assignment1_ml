...
class TestFeatures(TestCase):
    ...
    def test_standard_scaler_fit_transform(self):
        ...
        expected_scaled_data = (data - expected_mean) / expected_std
        scaler = StandardScaler()
        result = scaler.fit_transform(data)
        assert np.allclose(result, expected_scaled_data), f"Scaler fit_transform does not return expected values. Expected: {expected_scaled_data}, Got: {result}"
    def test_standard_scaler_get_standard_deviation(self):
        scaler = StandardScaler()
        data = [[1, 2], [3, 4], [5, 6], [7, 8]]
        expected_std = np.std(data, axis=0)
        scaler.fit(data)
        assert (scaler.std == expected_std).all(), "scaler fit does not return expected mean {}. Got {}".format(expected_std, scaler.std)    
